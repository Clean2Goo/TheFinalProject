							...
							<div class="item-photo-book">
								<c:choose>
									<c:when test="${not empty carWashDetail.washImg}">
										<img src="${carWashDetail.washImg}" alt="${carWashDetail.washName}" alt="${carWashDetail.washName}"/>
									</c:when>
									<c:otherwise>
										<img src="${contextPath}/resources/assets/images/default-carwash.jpg" alt="이미지 준비중" />
									</c:otherwise>
								</c:choose>
							</div>
						</div>
