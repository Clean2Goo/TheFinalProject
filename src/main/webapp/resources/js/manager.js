document.addEventListener("DOMContentLoaded", () => {
	...
	form.addEventListener("submit", async (e) => {
		...
		const endpoint = carwashData.washId ? `${contextPath}/api/manager/carwashes/${carwashData.washId}` : `${contextPath}/api/manager/carwashes`;
	...
	function renderCarwashList(carwashes) {
		...
		carwashes.forEach((carwash) => {
			...
			row.innerHTML = `
                ...
                <td>
					<div class="table-flex-box span-exception">
						<button class="ux-button contained button-table primary edit-btn" data-id="${carwash.washId}">수정</button>
						<button class="ux-button contained button-table remove delete-btn" data-id="${carwash.washId}">삭제</button>
					</div>
                </td>
            `;
		...
		// 삭제 버튼 이벤트 연결
		document.querySelectorAll(".delete-btn").forEach((btn) => btn.addEventListener("click", (e) => deleteCarwash(e.target.dataset.id)));
	}
