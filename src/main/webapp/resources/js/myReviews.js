	...
	function renderReviewList(reviews) {
		...
		reviews.forEach((review) => {
			...
			row.innerHTML = `
                ...
                <td class="tal">${review.content || "내용 없음"}</td>
                <td>${review.rwvScore || "평점 없음"}점</td>
                <td>${formattedDate}</td>
                <td>
					<div class="table-flex-box span-exception">
						<button class="ux-button contained button-table primary edit-button" data-id="${review.rwId}">수정</button>
						<button class="ux-button contained button-table remove delete-button" data-id="${review.rwId}">삭제</button>
					</div>
                </td>
            `;
